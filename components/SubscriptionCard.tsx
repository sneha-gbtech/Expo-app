...
const SubscriptionCard = ({ name, price, currency, icon, billing, color, category, plan, renewalDate, expanded, onPress, onSharePress, paymentMethod, startDate, status }: SubscriptionCardProps) => {
    ...
    return (
        <Pressable onPress={onPress} className={clsx('sub-card', expanded ? 'sub-card-expanded' : 'bg-card')} style={!expanded && color ? { backgroundColor: color } : undefined}>
            <Pressable onPress={onSharePress} className="absolute top-3 right-3">
